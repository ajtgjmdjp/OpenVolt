...
std::vector<TaxCostEstimate> USTaxPolicy::estimate_tax_costs(
    ...
    for (const auto& [ticker, lots] : lots_by_ticker) {
        ...
        // For US, compute weighted average tax cost considering lot-level gains
        double tax_cost = 0.0;
        for (const auto& lot : lots.lots()) {
            double gain = lot.shares * (price - lot.cost_per_share);
            // Use today's date approximation for long/short determination
            // In practice, this should use the actual sale date
            double rate = lot.cost_per_share < price
                ? short_term_rate_  // Conservative: assume short-term for positive gains
                : 0.0;              // Losses are beneficial regardless
            if (gain > 0.0) {
                tax_cost += gain * rate;
            } else {
                // Losses reduce tax cost (incentive to harvest)
                tax_cost += gain * short_term_rate_;  // Negative value = benefit
            }
        }
